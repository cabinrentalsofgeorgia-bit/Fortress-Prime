...
| **Wave 3 v2 partial (2026-05-01)** | **PARTIAL PASS** — EMBED verified (spark-3:8102, llama-nemotron-embed-1b-v2, dim=2048), e2e embed→qdrant smoke 64ms wall, frontier 200 throughout. Reranker + Extraction NIMs deferred (see Wave 3.5 watchlist below). Conventions captured at `docs/operational/cluster-nim-deployment-conventions.md`. | this PR |
| **Wave 3.5: Reranker `llama-3.2-nv-rerankqa-1b-v2:1.8.0`** | DEFERRED — pull + ARM64-verify clean, service starts healthy, but inference fails at ReduceSum ONNX kernel with `cudaErrorSymbolNotFound` (same family as embed v1.10.0, forums 354998). **Gate:** NIM rebuild against Spark-compatible CUDA/ONNX runtime. Tar + verification record retained at `/mnt/fortress_nas/nim-cache/nim/llama-3.2-nv-rerankqa-1b-v2/1.8.0/`; unit retained at `deploy/systemd/spark-5/fortress-nim-rerank.service`. Zero-touch re-enable when fixed. | Monitor NVIDIA forums 354998 |
| **Wave 3.5: Extraction NIMs (page/graphic/table)** | DEFERRED — all three (`nv-yolox-page-elements-v2`, `-graphic-elements-v1`, `-table-structure-v1`) return 403 DENIED at registry across every tag tried. **Gate:** NGC subscription upgrade for YOLOX entitlement (separate from technical fix — billing/sub axis). | Operator NGC ticket |
| **Wave 3.5: nv-ingest unified extraction orchestrator** | DEFERRED per brief v2 §17 — no public ARM64 build (NVIDIA staff aniculescu, forums 360011, 2026-02-09). **Gate:** ARM64 build availability. Independent of YOLOX entitlement gate above. | Monitor NVIDIA forums 360011 |
| **Wave 3.5: Vision restart on spark-3** | DEFERRED — spark-3 GPU has only 16.18 GiB free of 121.69 GiB (TP=2 frontier dominates); Vision NIM requests 0.55 utilization = 66.93 GiB → engine init rejection. Brief constraint forbids Vision config modification. **Gate:** spark-3 capacity headroom restoration via (a) frontier relocation off spark-3, (b) frontier KV cache reduction, or (c) explicit brief authorizing Vision `gpu_memory_utilization` reduction. Service stopped (failed/enabled). | Future brief |
| **Wave 3.5: `legal_ediscovery` reindex** | DEFERRED — collection is 768-dim (738,918 points) but current EMBED is 2048-dim. Cluster pattern is parallel `*_v2` reindex (e.g., `legal_privileged_communications` and `*_v2` both at 241,167 pts). 738K-point reindex needs its own brief: source-of-truth re-discovery, throughput planning, atomic alias swap (qdrant#7584), caller migration to `legal_ediscovery_active` alias. Helper exists at `src/reindex_legal_qdrant_to_legal_embed.py`. | Future brief |
| **Wave 3.5: `llama-nemotron-rerank-1b-v2` (newer model)** | DEFERRED per brief v2 §17 — model card explicitly lists Ampere/Hopper/Lovelace only, no Blackwell. **Gate:** NVIDIA publishes Blackwell support for the newer rerank model. Independent of `:1.8.0` ONNX/CUDA gate above. | Monitor NVIDIA model cards |
