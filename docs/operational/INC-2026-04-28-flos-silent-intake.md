...
8. ~~**Tag gary-gk and gary-crog with `ingester=legal_mail`** in MAILBOXES_CONFIG so real Case I/II correspondence flows through the new ingester (separate config-only PR — pending operator decision).~~ **DONE 2026-04-28 (FLOS Phase 0a-10)** — production `.env` updated (`ingester=legal_mail` + `routing_tag` flipped `executive`→`legal` for both, per Path 3 rationale); docs PR #274 merged. Three mailboxes now patrolling; gary-crog surfaced 6 watchdog matches in first 3 cycles, validating the §3.5 design hypothesis. See `docs/operational/mailbox-routing.md`.
...
10. **M3 activation prerequisites (spark-1 readiness)** — Phase M3 spark-1 mirror code merged 2026-04-28 (PR #268, default OFF) but activation is deferred. Diagnostic 2026-04-28 sharpened the blocker list:
    - **(a) Memory headroom: currently fine, but conditional.** The CLAUDE.md 2026-04-23 note ("BRAIN at ≥99% of 121 GiB") does not reflect spark-1's current state — `free -h` shows 52 GiB available (43% headroom). Verified running services on spark-1: `fortress-nim-sovereign.service` (Llama-3.1-8b, ~200 MB RSS), `fortress-brain.service` (Streamlit), Ray workers, Ollama. The 49B Nemotron BRAIN described in CLAUDE.md is NOT running on spark-1 (per operator: it runs on spark-5). Capacity planning for M3 dual-write depends on which BRAIN deployment is canonical going forward — see item 11.
    - **(b) Postgres on spark-1 is NOT running on :5432.** Hard blocker. `nc -zv spark-node-1 5432` → connection refused on both LAN (10.10.10.1) and Tailscale (100.127.241.36). Activation cannot begin until Postgres is installed/started/configured to accept LAN connections from spark-2.
    - **(c) Alembic chain risk.** 30 of 121 migrations reference `GRANT`/`CREATE ROLE`/`CREATE USER`. Fresh-DB `alembic upgrade head` against a clean `fortress_prod` on spark-1 will hit each one; if any reference roles created out-of-band (Issue #204 precedent on spark-2), the chain breaks. Runbook Step 3 does not yet document a role-bootstrap script. Likely needs the same raw-psql apply pattern Issue #204 documents for spark-2.

    Activation runbook: `docs/runbooks/m3-spark1-mirror-activation.md`.

11. **BRAIN tier ambiguity — CLAUDE.md drift from production reality.** CLAUDE.md (DEFCON 3 / BRAIN section) describes `fortress-nim-brain.service` running `nvidia/Llama-3.3-Nemotron-Super-49B-v1.5-FP8` on spark-1 with vLLM, port 8100, 32k context, expected ≥99% of 121 GiB. Verified on 2026-04-28: that service does not exist on spark-1; the actual sovereign LLM tier on spark-1 is `fortress-nim-sovereign.service` running Llama-3.1-8b (~200 MB RSS). Per operator: 49B Nemotron deployment is on spark-5. The CLAUDE.md guidance is unreliable for capacity-planning decisions until corrected. Specifically: the 2026-04-23 "do not put production traffic on BRAIN until Track B completes" line was driving M3 deferral assumptions that don't apply to spark-1's actual state. Action: update CLAUDE.md to reflect production reality (or annotate the stale section with a date-stamped "as of <date>, no longer current" pointer to a follow-up doc). Cannot be done without operator decision on the BRAIN tier — its own ticket. Until corrected, treat CLAUDE.md memory/BRAIN guidance as advisory-not-authoritative; verify against the host before acting on it.
