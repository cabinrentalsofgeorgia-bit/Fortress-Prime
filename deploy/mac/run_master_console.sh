#!/bin/bash
# Wrapper for launchd: sources .env and .env.security (JWT RSA keys, DB, etc.)
# then runs master_console.py.
...
[[ -f "${REPO}/.env.security" ]] && set -a && source "${REPO}/.env.security" && set +a
