...
.legacy-homepage-fallback #search-bar {
  margin-top: 24px;
}
...
const getLegacyHomepageMarkup = cache(async () => {
  try {
...
export default async function StorefrontHomepage() {
  const [{ allProperties }, legacyMarkup] = await Promise.all([
    getStorefrontHomeData(),
    getLegacyHomepageMarkup(),
  ]);
  ...
  return (
    <>
      ...
      <div className="legacy-homepage">
        ...
        {legacyMarkup ? <div dangerouslySetInnerHTML={{ __html: legacyMarkup.bottom }} /> : null}
      </div>
    </>
  );
}
