...
export function ReviewOperationsPanel({ slug }: { slug: string }) {
  ...
  return (
    <div className="rounded-lg border border-cyan-500/30 bg-cyan-500/5 p-4 space-y-4">
      ...
      <section className="rounded-md border border-sky-500/30 bg-sky-500/5 p-3 space-y-3">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <p className="text-xs font-semibold text-sky-100 flex items-center gap-2">
            <Activity className="h-3.5 w-3.5 text-sky-300" />
            Controlled Internal Pilot Operations
          </p>
          <Badge variant="outline" className="bg-sky-500/10 text-sky-300 border-sky-500/30 text-[10px]">
            {label(data.internal_pilot.status)}
          </Badge>
        </div>
        <p className="text-[10px] text-zinc-500">
          {label(data.internal_pilot.pilot_mode)} / {label(data.internal_pilot.execution_scope)}.
        </p>
        <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-6">
          <Metric label="Pilot Queue" value={data.internal_pilot.pilot_summary.review_queue_depth} />
          <Metric label="Remediation Triage" value={data.internal_pilot.pilot_summary.remediation_triage_items} />
          <Metric label="Contradiction Review" value={data.internal_pilot.pilot_summary.contradiction_review_items} />
          <Metric label="Evidence Navigation" value={data.internal_pilot.pilot_summary.evidence_navigation_items} />
          <Metric label="Escalations" value={data.internal_pilot.pilot_summary.escalation_items} />
          <Metric label="Excluded Sources" value={data.internal_pilot.pilot_summary.excluded_source_issues} />
        </div>
        <div className="grid gap-3 xl:grid-cols-3">
          <div className="rounded border border-zinc-800 bg-zinc-950/70 p-2">
            <p className="text-[11px] font-semibold text-zinc-100">Allowed Pilot Exercises</p>
            <div className="mt-1 flex flex-wrap gap-1">
              {data.internal_pilot.allowed_exercises.map((exercise) => (
                <Badge key={exercise} variant="outline" className="bg-blue-500/10 text-blue-300 border-blue-500/30 text-[10px]">
                  {label(exercise)}
                </Badge>
              ))}
            </div>
          ...
          <div className="rounded border border-zinc-800 bg-zinc-950/70 p-2">
            <p className="text-[11px] font-semibold text-zinc-100">Forbidden Pilot Actions</p>
            <div className="mt-1 flex flex-wrap gap-1">
              {data.internal_pilot.forbidden_exercises.map((exercise) => (
                <Badge key={exercise} variant="outline" className="bg-red-500/10 text-red-300 border-red-500/30 text-[10px]">
                  forbidden {label(exercise)}
                </Badge>
              ))}
            </div>
          </div>
          <div className="rounded border border-zinc-800 bg-zinc-950/70 p-2">
            <p className="text-[11px] font-semibold text-zinc-100">Pilot Simulation / Drills</p>
            <div className="mt-1 space-y-1">
              {data.internal_pilot.pilot_drills.map((drill) => (
                <div key={drill.scenario} className="rounded border border-zinc-800 bg-zinc-900/70 px-2 py-1 text-[10px] text-zinc-300">
                  {label(drill.scenario)} / {label(drill.expected_response)}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid gap-3 xl:grid-cols-2">
          <div className="rounded border border-zinc-800 bg-zinc-950/70 p-2">
            <p className="text-[11px] font-semibold text-zinc-100">Pilot Throughput Metrics</p>
            <div className="grid gap-2 sm:grid-cols-3 mt-1">
              <Metric label="Traversal Sample" value={data.internal_pilot.throughput_metrics.review_traversal_sample} />
              <Metric label="Reviewer Handoffs" value={data.internal_pilot.throughput_metrics.reviewer_handoff_count} />
              <Metric label="Completion" value={label(data.internal_pilot.pilot_summary.pilot_completion_readiness)} />
            </div>
            <div className="mt-1 flex flex-wrap gap-1">
              {data.internal_pilot.throughput_metrics.queue_aging_bands.map((row) => (
          ...
          <div className="rounded border border-zinc-800 bg-zinc-950/70 p-2">
            <p className="text-[11px] font-semibold text-zinc-100">Throughput Optimizations</p>
            <div className="mt-1 flex flex-wrap gap-1">
              {data.internal_pilot.ergonomic_optimizations.map((optimization) => (
                <Badge key={optimization} variant="outline" className="bg-emerald-500/10 text-emerald-300 border-emerald-500/30 text-[10px]">
                  {label(optimization)}
                </Badge>
              ))}
              <Badge variant="outline" className="bg-zinc-900 text-zinc-300 border-zinc-700 text-[10px]">
                production writes {label(data.internal_pilot.governance.production_writes)}
              </Badge>
            </div>
          </div>
        </div>
