...
from backend.models.legal import LegalExemplar
...
async def get_godhead_exemplars(
    db: AsyncSession,
    categories: list[str] | None = None,
    max_items: int = 3,
) -> list[dict]:
    """Fetch random high-quality exemplars from the legal exemplar memory bank."""
    query = (
        select(
            LegalExemplar.category,
            LegalExemplar.rationale_context,
            LegalExemplar.perfect_output,
        )
        .order_by(func.random())
        .limit(max_items)
    )
    if categories:
        query = query.where(LegalExemplar.category.in_(categories))
