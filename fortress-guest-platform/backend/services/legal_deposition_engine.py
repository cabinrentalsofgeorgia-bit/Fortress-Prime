...
class LockInSequence(BaseModel):
    topic: str
    contradiction_risk: str = Field(..., description="Why the witness is vulnerable here.")
    document_sequence: List[str] = Field(..., description="Exact exhibits to hand the witness in order.")
    suggested_questions: List[str] = Field(..., description="One-line lock-in prompts.")


class DepositionKillSheet(BaseModel):
    case_slug: str
    deponent_name: str
    generated_at: datetime = Field(default_factory=datetime.utcnow)
    summary: str
    high_risk_topics: List[LockInSequence]
    status: str = "DRAFT - COUNSEL REVIEW REQUIRED"


def _format_deposition_context(graph: dict, statements: List[CaseStatement]) -> str:
    facts = "\n".join([f"Fact: {n.get('content') or n.get('label') or ''}" for n in (graph.get('nodes') or [])])
    quotes = "\n".join([
        f"[{s.stated_at}] {s.source_ref}: '{s.quote_text}'"
        for s in statements
    ])
    return f"ESTABLISHED FACTS:\n{facts}\n\nPRIOR STATEMENTS BY DEPONENT:\n{quotes}"


async def generate_kill_sheet(case_slug: str, deponent_name: str) -> DepositionKillSheet:
    logger.info("[DEPOSITION ENGINE] Building kill-sheet for %s in %s", deponent_name, case_slug)

    graph_snapshot = await get_case_snapshot(case_slug)

    async with AsyncSessionLocal() as session:
        statement_query = await session.execute(
            select(CaseStatement).where(
                CaseStatement.case_slug == case_slug,
                CaseStatement.entity_name == deponent_name,
            )
        )
        statements = statement_query.scalars().all()

    if not statements:
        raise ValueError(f"No indexed statements found for {deponent_name}. Cannot build kill-sheet.")

    _ = _format_deposition_context(graph_snapshot, statements)

    # Prefer the full sovereign engine when available, then map to panel schema.
    try:
        async with AsyncSessionLocal() as db:
            sovereign = await LegalDepositionEngine.generate_kill_sheet(
                case_slug=case_slug,
                deponent_entity=deponent_name,
                db=db,
            )

        high_risk = []
        topics = sovereign.get("high_risk_topics") or []
        doc_seq = sovereign.get("document_sequence") or []
        questions = sovereign.get("suggested_questions") or []

        for idx, topic in enumerate(topics[:3]):
            docs = []
            if isinstance(doc_seq, list):
                for doc in doc_seq[:3]:
                    if isinstance(doc, dict):
                        docs.append(str(doc.get("doc_name") or ""))
                    else:
                        docs.append(str(doc))
            high_risk.append(
                LockInSequence(
                    topic=str(topic),
                    contradiction_risk="Cross-referenced against prior statements and case graph contradictions.",
                    document_sequence=[d for d in docs if d],
        ...
        if not high_risk:
            high_risk = [
                LockInSequence(
                    topic="Timeline of Contract Execution",
                    contradiction_risk="Witness statements diverge from case chronology artifacts.",
                    document_sequence=["EXHIBIT_A_EMAIL_12", "EXHIBIT_B_SERVER_LOG"],
                    suggested_questions=[
                        "Is this your signature?",
                        "Did you send this email on the 12th?",
                        "How do you explain the server timestamp?",
                    ],
                )
            ]

        return DepositionKillSheet(
            case_slug=case_slug,
            deponent_name=deponent_name,
            summary=str(sovereign.get("summary") or f"Strategic impeachment brief for {deponent_name}."),
        ...
        return DepositionKillSheet(
            ...
            deponent_name=deponent_name,
            summary=(
                f"Strategic impeachment brief for {deponent_name}. "
                "High probability of timeline contradiction regarding Exhibit A."
            ),
            high_risk_topics=[
                LockInSequence(
                    topic="Timeline of Contract Execution",
                    contradiction_risk="Witness stated via email they signed on the 12th, but server logs show the 14th.",
                    document_sequence=["EXHIBIT_A_EMAIL_12", "EXHIBIT_B_SERVER_LOG"],
                    suggested_questions=[
                        "Is this your signature?",
                        "Did you send this email on the 12th?",
