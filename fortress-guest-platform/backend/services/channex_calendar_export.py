...
Uses the same sellability inputs as the local availability reads: blocked days,
occupying reservations, and active non-expired holds.
        ...
        await db.execute(
            select(Reservation.check_in_date, Reservation.check_out_date).where(
                and_(
                    ...
                    Reservation.status.in_(CALENDAR_BLOCKING_RESERVATION_STATUSES),
