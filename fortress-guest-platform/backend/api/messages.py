...
_LITELLM_CHAT = f"{getattr(settings, 'litellm_base_url', 'http://127.0.0.1:4000/v1').rstrip('/')}/chat/completions"
...
async def draft_ai_reply(
    ...
    """Draft an AI reply for a guest message using the God Head SWARM cluster."""
    message = await db.get(Message, message_id)
    if not message:
        raise HTTPException(status_code=404, detail="Message not found")

    system_prompt = (
        "You are a luxury concierge for Cabin Rentals of Georgia. "
        "Draft a warm, professional, and concise text message reply to this guest inquiry. "
        "Do not include placeholders, just the exact text to send."
    )
    user_content = f"Guest message:\n\n{message.body or '(empty message)'}"
    ...
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.post(GODHEAD_URL, json=payload, timeout=45.0)
            resp.raise_for_status()
    except httpx.TimeoutException:
        raise HTTPException(status_code=504, detail="God Head LLM timed out (>45s)")
    except httpx.RequestError as exc:
        _log.warning(f"God Head request failed: {exc}")
        raise HTTPException(status_code=502, detail="God Head LLM cluster unreachable")
    latency_ms = int((time.perf_counter() - start) * 1000)

    data = resp.json()
    draft = ""
    for choice in data.get("choices") or []:
        draft = (choice.get("message") or {}).get("content") or ""
        if draft:
            break
    draft = draft.strip() or "(LLM returned empty response)"
