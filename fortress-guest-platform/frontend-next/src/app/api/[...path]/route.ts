...
function buildUpstreamHeaders(
  ...
  if (isCC && token) {
    // Command Center requires fortress_session cookie for auth.
    // Inject it if not already present in the forwarded cookies.
...
async function proxy(
  ...
  try {
    ...
    // Build response headers — forward everything the client needs
    const responseHeaders = new Headers();
    responseHeaders.set("Content-Type", contentType);

    // Forward Set-Cookie so the browser picks up session cookies from upstream.
    // This is critical for the Command Center login flow which sets
    // fortress_session via Set-Cookie.
    const setCookies =
