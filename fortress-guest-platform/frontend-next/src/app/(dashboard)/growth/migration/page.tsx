...
function deriveNextRoute(sourcePath: string, alias: string): string {
  ...
  if (alias.startsWith("/testimonial")) return `/reviews${alias.slice("/testimonial".length)}`;
  ...
  function walkMenu(items: typeof menus[string], menuName: string) {
    for (const item of items) {
      if (item.link_path && item.link_path !== "<nolink>") {
        entries.push({
          alias: item.link_path.startsWith("/") ? item.link_path : `/${item.link_path}`,
          sourcePath: item.link_path,
          contentType: "Menu",
          nodeType: menuName,
          title: item.title || item.link_path,
          nextRoute: deriveNextRoute(
            item.link_path,
            item.link_path.startsWith("/") ? item.link_path : `/${item.link_path}`
          ),
        });
      }
...
export default function MigrationPage() {
  ...
  const stats = useMemo(() => {
    const counts = { Node: 0, Taxonomy: 0, Menu: 0 };
    ...
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <Card>
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">Node Pages</p>
              <ExternalLink className="h-3.5 w-3.5 text-blue-400" />
            </div>
            <p className="text-xl font-bold font-mono">{stats.Node.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3">
            <div className="flex items-center justify-between">
              <p className="text-xs text-muted-foreground">Taxonomy Terms</p>
              <ExternalLink className="h-3.5 w-3.5 text-purple-400" />
            </div>
            <p className="text-xl font-bold font-mono">{stats.Taxonomy.toLocaleString()}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3">
            ...
            <p className="text-xl font-bold font-mono">{stats.Menu.toLocaleString()}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <CardTitle className="text-base">Legacy URL Migration Map</CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-xs text-muted-foreground uppercase text-right pr-4">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {slice.length === 0 ? (
                ...
                slice.map((entry, i) => (
                  <TableRow key={`${entry.sourcePath}-${i}`}>
                    ...
                    <TableCell className="font-mono text-xs text-emerald-500 max-w-[280px] truncate">
                      {entry.nextRoute}
                    </TableCell>
