...
export function SovereignQuoteWidget({
  ...
  const continueHref =
    quoteState?.quote && quoteState.ok && checkIn && checkOut
      ? `/book?propertyId=${encodeURIComponent(propertyId)}&checkIn=${encodeURIComponent(checkIn)}&checkOut=${encodeURIComponent(checkOut)}&guests=${encodeURIComponent(String(totalGuests))}&pets=${encodeURIComponent(String(pets))}`
      : "/book";
    ...
    >
      <div className="space-y-3">
        ...
        <p className="text-sm leading-7 text-slate-600">
          Live quote math is calculated from the sovereign pricing engine through a secure{" "}
