...
check_doc() {
  ...
  if [[ -f "$REPO_ROOT/$path" ]]; then
    ...
    hard_fail "missing required doc $path"
  fi
}
...
check_service() {
  ...
  if [[ "$status" == "active" ]]; then
    ...
    warn "service $unit status=$status"
  else
...
section "Identity"
printf 'script=fortress-legal-health\n'
...
df -h / "$REPO_ROOT" 2>/dev/null | redact || warn "disk check unavailable"
...
check_doc "docs/production/runtime-lineage.md"
check_doc "docs/architecture/system-topology.md"
check_doc "docs/auth/auth-boundaries.md"
check_doc "docs/database/supabase-classification.md"
check_doc "docs/runbooks/cli-agent-operating-rules.md"
...
if [[ "$HARD_FAILURES" -gt 0 ]]; then
  printf 'FORTRESS_LEGAL_HEALTH_STATUS=FAIL\n'
  exit 1
fi
printf 'FORTRESS_LEGAL_HEALTH_STATUS=OK_WITH_%s_WARNINGS\n' "$WARNINGS"
